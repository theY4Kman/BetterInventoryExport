...
void BetterInventoryExport::OnInventDump(std::vector<std::string> params)
{
    ...
    std::filesystem::path destinationFile = gameWrapper->FixRelativePath("./bakkesmod/data/inventory." + outputFormat);
    if (outputFormat == "csv")
    {
...
ProductStruct BetterInventoryExport::GetOnlineProductInfo(OnlineProductWrapper& unlockedProduct)
{
    ...
    prodData.series_id = unlockedProduct.GetSeriesID();
    prodData.series = unlockedProduct.GetProductSeries();
...
void BetterInventoryExport::export_csv(const std::filesystem::path& filename, std::vector<ProductStruct>& psv)
{
    ...
    myfile << "product id,name,slot,paint,certification,certification value,certification label,quality,series_id,series,tradeable,amount,instanceid,specialedition,blueprint item id, blueprint item, blueprint cost" << std::endl;
...
void BetterInventoryExport::export_json(const std::filesystem::path& filename, std::vector<ProductStruct>& psv)
{
    ...
    for (auto it = psv.begin(); it != psv.end(); it++)
    {
        ...
        EXP_VAL("series_id", ps.series_id);
        EXP_STRING("series", ps.series);
